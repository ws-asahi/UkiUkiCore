...
| 電源 | USB 5V / DC ジャック(DC7-24V / バックコンバーター)|
...
| `Serial2` | USART1 | D3(RX) / D2(TX) | 予追加 UART |
...
- **D0 / D2 / D8** - TCB1 の 8bit PWM 波形を LUT もしくは直接出力(排他使用)
...
| メニュー | F_CPU | USB シリアル | 主な用途 |
|---------|-------|:-----------:|---------|
| 24 MHz internal(既定) | 24 MHz | 可 | 通常はこちら |
| 16 MHz internal | 16 MHz | 可 | classic AVR(16 MHz)とのタイミング互換、省電力 |

- **16 MHz でも USB がそのまま使えます。** USB は CLK_PER 最低 12 MHz を要求します
  (DS40002548B 28.3.1.1)が、16 MHz はこれを満たします。また USB クロック(48 MHz)は
  OSCHF から PLL48M への**固定 4 MHz 出力**から作られており、メインクロックの
  周波数選択とは独立しているため影響を受けません(同 12.3.4.1.1)。
...
| 電源 | JP1はんだ |  | VCC 電圧 | MCU 電圧 |
|------|------|------|------|------|
| USB バスパワー | 5V - VCC | -> | 5V | 5V |
| USB バスパワー | 3.3V - VCC | -> | 3.3V | 3.3V |
| VIN (5V) | 5V - VCC | -> | 5V | 5V |
| VIN (5V) | 3.3V - VCC | -> | 3.3V | 3.3V |
...
| LED_BUILTIN_RX | 橙 | D11(Active-LOW) | USB-CDC 送信アクティビティ |
| LED_BUILTIN_TX | 橙 | D12(Active-LOW) | USB-CDC 送信アクティビティ |
| LED_BUILTIN | - | D12(Active-LOW) | 実体は無いが互換性のためRXに接続 |
...
| マクロ | 定義元 | 用途 |
|--------|--------|------|
| `ARDUINO_AVR_KUNAI` | Arduino ビルドシステム(`boards.txt` の `build.board=AVR_KUNAI`)。IDE 外ビルド用に variant にもフォールバック定義あり | **ボード識別**(Arduino 慣例の `ARDUINO_<board>` 形式、`ARDUINO_AVR_PROMICRO` 相当)。他ボードは `ARDUINO_AVR_TACHI` / `ARDUINO_AVR_TSURUGI` / `ARDUINO_AVR_KUNAI` |
...
1. **LED_BUILTIN なし** : 独立した LED が無く 代わりに RX LED が動作します。
2. **SWCLK / SWDIO なし** : USB 端子裏のパッドがSWCL, SWDIOではなく、代わりに UPDI と HV RESET (UPDI v2 用パッド) になっています。
3. **5V動作が可能** : 5V 動作時には XIAO の 3.3V 供給ピンに 5V が供給されるため、耐圧 3.3V の周辺機器は破壊される可能性があります。
4. **PWM は最大7系統** : D1, D3 は PWM に完全非対応。D0, D2, D8 の PWM は排他式。
5. **一部ピンで ADC なし** : A6, A7 ピンは ADC なし。
6. **変数サイズの不一致** : int は 4 -> 2 バイト。double は 8 -> 4バイト(long doubleは 8 バイトで一致)。 
