...
// UkiUkiCore (AVR DU, modern AVR):
//  The SPI peripheral offers only /2, /4, /8 ... prescalers, so on a 24 MHz
//  device the stock 14 MHz request rounds to 12 MHz.  A W5100 shield does
//  not respond correctly at 12 MHz (register reads come back shifted by one
//  bit - verified on UkiUkiduino + W5100 Ethernet shield), while 6 MHz and
//  8 MHz (Uno R3) are fine.  Requesting 8 MHz gives 6/8/5/6 MHz for
//  F_CPU = 12/16/20/24 MHz, which is safe for W5100 on every supported clock.
//  The W5200/W5500 could go faster but the difference is not worth a second
//  code path; see megaavr/extras/UkiUkiduino.md (SPI section).
...
private:
#if defined(ARDUINO_ARCH_MEGAAVR)
	// UkiUkiCore: modern AVR PORT has OUTSET/OUTCLR right after OUT
	// (PORT_t: OUT +0, OUTSET +1, OUTCLR +2), so the chip select can be
	// driven with single atomic writes instead of a read-modify-write.
	...
	inline static void setSS() {
		*(ss_pin_reg + 2) = ss_pin_mask;   // OUTCLR
	}
	inline static void resetSS() {
		*(ss_pin_reg + 1) = ss_pin_mask;   // OUTSET
	}
