 ...
 *
 * このスケッチでは、オンボードボタン(BTN_BUILTIN = D32)の状態を
 * イベント出力ピンD2へ届けます。D2にLED(+抵抗、GNDへ)を接続すると、
 * ボタンを押している間だけLEDが点灯します。
 * (UkiUkimicroでも BTN_BUILTIN = D32、イベント出力ピンは D8 です)
 *
 * メモ: connect()は入力側ピンに内部プルアップを設定しますが、D32には
...
void setup() {
  ...
  // ボタン -> EVOUTA。EVOUTAはUkiUkiduinoでD2、UkiUkimicroでD8。
