 ...
 * DxCore; the pin map follows the UkiUkiduino variant).
 ...
 *
 * Board   : UkiUkiduino (Arduino Uno R3 form factor, AVR64DU32, USB-C;
 *           official fan goods of the VTuber Yuniko Ukiuki)
 ...
 *           PA0/PA1 (the XTALHF pads) are repurposed as the on-board WS2812
 *           RGB LED data line (PA0) and the on-board user button (PA1).
 *
 ...
 *   D13  PD6   SPI SCK | LED_BUILTIN (WS2812 mirror, see LED)  A19, AIN6
 ...
 *   LED   -> on-board LED is a WS2812D-F5-12mA-C1 addressable RGB LED, data-in
 *           on PA0. digitalWrite()/digitalWriteFast() to D13 (PD6) are
 *           SOFTWARE-mirrored: the core reads the resulting PD6 OUT bit and
 *           sends the matching WS2812 frame (HIGH = lit in the current color,
 *           yellow by default; LOW = off), so the stock Blink sketch works
 *           unmodified. setBLEDColor() (below) changes the lit color/brightness.
...
/* Board identification: ARDUINO_AVR_UKIUKIDUINO is THE macro that identifies
 * this board, following the Arduino convention (ARDUINO_AVR_PROMICRO, ARDUINO_UNOWIFIR4).
 * The Arduino build system defines it from boards.txt (build.board=AVR_UKIUKIDUINO
 * -> -DARDUINO_AVR_UKIUKIDUINO); the fallback below covers builds outside the IDE
...
#ifndef ARDUINO_AVR_UKIUKIDUINO
  #define ARDUINO_AVR_UKIUKIDUINO 1
#endif
#define DU_32PIN_PINOUT
#define UKIUKIDUINO_PINOUT   /* used by ukiukiduino_init.cpp / ukiukiduino_led.cpp */
...
#define PIN_PD6 (13)  // D13 SPI SCK / LED_BUILTIN (mirrored to the WS2812 LED on PA0)
...
#endif
#define HWSERIAL0_MUX                   (0x00 /* PORTMUX_USART0_DEFAULT_gc - PA0/PA1 = LED data / button */)
...
#define HWSERIAL0_MUX_DEFAULT          (1)   /* UkiUkiduino default: USART0 ALT1 (PA4/PA5). DEFAULT(PA0/PA1) = LED data / button. */
...
#define HWSERIAL1_MUX_DEFAULT          (2)   /* UkiUkiduino: USART1 fixed on ALT2 (PD6/PD7 = D13/D20) */
 ...
 *  Effective app identity: 0x1209:0x000C, product "UkiUkiduino".
